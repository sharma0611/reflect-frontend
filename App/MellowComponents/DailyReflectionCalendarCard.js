...
const DayComponent = ({ date, state, navigation }) => {
    ...
    const onDayPress = async () => {
        const isComplete = await ActivityResponse.isDailyReflectionCompleted(dt)
        const activity = await ActivityResponse.dailyReflection(dt)
        if (!hasPro) {
            return navigation.navigate('MellowPaywall')
        }
        if (!isComplete) {
            // update the reflection with the mood from the other screen
            navigation.navigate('Activity', { activity, index: 0 })
        } else {
    ...
    const inFuture = moment().diff(moment(dt), 'days') < 0
    return (
        <Touchable onPress={onDayPress} disabled={inFuture}>
            <V
                key={date.dateString}
                ai="center"
                jc="center"
...
const ReflectionCalendar = ({ navigation }) => {
    return (
        <Card alt bg="WhiteM">
            <Calendar
...
export default withNavigation(ReflectionCalendar)
